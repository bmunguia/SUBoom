...
public:
  ...
  double z[25001], T_of_z[25001], a_of_z[25001], p_of_z[25001], rho_of_z[25001];
  ...
  SUBoom();
  SUBoom(double h0, double M, double psi, double gamma, double g,
  ...
  void ScaleFactors(double x[], int len);
  void InitialWaveNormals();
  void RayTracer();
  void RayTubeArea();
  void FindInitialRayTime();
  void ODETerms();
  void DistanceToTime(double x[], double T[], int len);
  void CreateSignature(double T[], double p[], int len); // Note: p is pressure disturbance (relative)
  void PropagateSignal(int len, int& Msig);
