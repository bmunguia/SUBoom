...
SUBoom::SUBoom(double h0, double M, double psi, double gamma, double g,
  ...
  this->flt_M = M;
  this->flt_psi = psi;
  this->flt_gamma = gamma;

  /*---Atmosphere variables---*/
  this->atm_g = g;
  this->atm_noise_flag = noise_flag;

  /*---Ray variables---*/
  this->ray_N_phi = N_phi;
  this->ray_phi = new double[N_phi];
  for(int i = 0; i < N_phi; i++){
    this->ray_phi[i] = phi[i];
  }
  this->ray_r0 = r0;
...
void SUBoom::ConditionAtmosphericData(){//SUBoom& boom){
  ...
  for(int i = 0; i < 25001; i++){
    z[i] = h0*double(i)/(25001.-1.);
    AtmosISA(z[i], T_of_z[i], a_of_z[i], p_of_z[i], rho_of_z[i], g);
  }
}

void SUBoom::ScaleFactors(double x[], int len){
  double L;
  double h0 = flt_h;
  double M_inf = flt_M;
...
void SUBoom::RayTracer(){
  ...
  double T = scale_T;
  double a0, rho0, p0;
  double a[25001], rho[25001], p[25001];
  //  double theta[25001];
  double r0[3];
  double *f, x[25001], y[25001], t[25001];
  double *kx, *ky, *kz;
  double dz = (z[0] - z[1]);
  ...
  for(int i = 0; i < ray_N_phi; i++){
    ...
    for(int j = 0; j < 4; j++){
      x_of_z[i][j] = new double[25001];
      y_of_z[i][j] = new double[25001];
      t_of_z[i][j] = new double[25001];
      dxdt[i][j] = new double[25001];
      dydt[i][j] = new double[25001];
      dzdt[i][j] = new double[25001];
      theta[i][j] = new double[25001];
    }
  }
   ...
   for(int i = 0; i < ray_N_phi; i++){
    ...
    kx = SplineGetDerivs(t, x, 25001);
    ky = SplineGetDerivs(t, y, 25001);
    kz = SplineGetDerivs(t, z, 25001);
    for(int ik = 0; ik < 25001; ik++){
      dxdt[i][0][ik] = kx[ik];
      dydt[i][0][ik] = ky[ik];
      dzdt[i][0][ik] = kz[ik];
    }
...
void SUBoom::ODETerms(){
  ...
  dadt = new double[25001];
  drhodt = new double[25001];
  dAdt = new double[25001];
  dcndt = new double[25001];
  ...
  for(int i = 0; i < ray_N_phi; i++){
    ...
    dadt = SplineGetDerivs(t, a_of_z, 25001);
    drhodt = SplineGetDerivs(t, rho_of_z, 25001);
    dAdt = SplineGetDerivs(t, A, 25001);
    dcndt = SplineGetDerivs(t, cn, 25001);

    ray_C1[i] = new double[25001];
    ray_C2[i] = new double[25001];
    for(int j = 0; j < 25001; j++){
    ...
    ray_dC1[i] = SplineGetDerivs(t, ray_C1[i], 25001);
    ray_dC2[i] = SplineGetDerivs(t, ray_C2[i], 25001);
  }
...
void SUBoom::DistanceToTime(double x[], double T[], int len){
  for(int i = 0; i < len; i++){
    T[i] = x[i]/(a_inf*flt_M);
  }
}

void SUBoom::CreateSignature(double T[], double p[], int len){
  double pp[2][len-1];
  double ll[len-1];
  double mm[len-1];
  ...
  for(int i = 0; i < len-1; i++){
    pp[0][i] = p[i]/scale_p;  // [Pa] (relative)
    pp[1][i] = p[i+1]/scale_p;
    ll[i] = (T[i+1] - T[i])/scale_T;  // [s]
  ...
  signal.original_T = new double[len];
  signal.original_p = new double[len];
  signal.M = M;
  ...
  for(int i = 0; i < len; i++){
    signal.original_T[i] = T[i];
    signal.original_p[i] = p[i];
  }
...
double *derivsProp(double t, int m, double y[], SUBoom::RayData data){
  ...
  C1 = EvaluateSpline(t,25001,data.t,data.C1,data.dC1);
  C2 = EvaluateSpline(t,25001,data.t,data.C2,data.dC2);
...
void SUBoom::PropagateSignal(int len, int& Msig){
  double t0, tf, dt;
  int j0;
  RayData data;
  double *fvec;
  ...
  for(int i = 0; i < ray_N_phi; i++){
    ...
    data.t = new double[25001];
    data.C1 = new double[25001];
    data.C2 = new double[25001];
    data.dC1 = new double[25001];
    data.dC2 = new double[25001];
